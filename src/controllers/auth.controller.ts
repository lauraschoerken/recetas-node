import { JsonController, Post, Get, Body, Req, UseBefore, HttpCode } from 'routing-controllers';
...
export class AuthController {
  ...
  async register(@Body() body: { email: string; password: string; name: string }) {
    ...
    try {
      ...
      throw { httpCode: 400, message: error.message || 'Error al registrar usuario' };
    }
  }
  ...
  async login(@Body() body: { email: string; password: string }) {
    ...
    try {
      ...
      throw { httpCode: 401, message: error.message || 'Credenciales inválidas' };
    }
  }
