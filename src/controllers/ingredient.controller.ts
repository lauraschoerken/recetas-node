...
export class IngredientController {
   ...
   */
  @Get('/nutrition')
  async getDailyNutrition(@QueryParam('date') dateStr: string, @Req() req: AuthRequest) {
  ...
  async create(
    @Body() body: { 
      ...
      unit?: string; 
      imageUrl?: string; 
      variants?: Array<{ name: string; isDefault?: boolean; calories?: number; protein?: number; carbs?: number; fat?: number; fiber?: number }>;
    ...
    const { name, unit, imageUrl, variants, calories, protein, carbs, fat, fiber } = body;
  ...
  async createBulk(
    ...
    if (validIngredients.length === 0) {
      throw { httpCode: 400, message: 'No hay ingredientes válidos para crear' };
    }
  ...
  async update(
    @Param('id') id: number,
    @Body() body: { name?: string; imageUrl?: string | null; preferredUnit?: string | null }
  ) {
    const { name, imageUrl, preferredUnit } = body;

    const ingredient = await ingredientService.update(id, { name, imageUrl, preferredUnit });
  ...
  async addVariant(
    ...
    @Body() body: { name: string; isDefault?: boolean; calories?: number; protein?: number; carbs?: number; fat?: number; fiber?: number; weightFactor?: number }
  ) {
    const { name, isDefault, calories, protein, carbs, fat, fiber, weightFactor } = body;
    ...
    return ingredientService.addVariant(ingredientId, { name, isDefault, calories, protein, carbs, fat, fiber, weightFactor });
  }
  ...
  async updateVariant(
    ...
    @Body() body: { name?: string; isDefault?: boolean; calories?: number | null; protein?: number | null; carbs?: number | null; fat?: number | null; fiber?: number | null; weightFactor?: number }
  ) {
  ...
  async deleteVariant(@Param('variantId') variantId: number) {
    ...
    if (!deleted) {
      throw { httpCode: 400, message: 'No se puede eliminar la variante (es la única o no existe)' };
    }
  ...
  async addConversion(
    ...
    if (!unitName || gramsPerUnit === undefined) {
      throw { httpCode: 400, message: 'unitName y gramsPerUnit son requeridos' };
    }

    if (typeof gramsPerUnit !== 'number' || gramsPerUnit <= 0) {
      throw { httpCode: 400, message: 'gramsPerUnit debe ser un número positivo' };
    }
    ...
    return ingredientService.addConversion(ingredientId, { unitName, gramsPerUnit });
  }
  ...
  async updateConversion(
    ...
    const conversion = await ingredientService.updateConversion(conversionId, gramsPerUnit);
