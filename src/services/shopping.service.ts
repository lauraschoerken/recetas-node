...
import { ShoppingItem, WeekPlanWithDetails, CreateWeekPlanDto } from '../domain';
...
export class ShoppingService {
  ...
  async getWeekPlan(userId: number, startDate: Date, endDate: Date): Promise<WeekPlanWithDetails[]> {
  ...
  async addToWeekPlan(data: CreateWeekPlanDto, userId: number): Promise<WeekPlanWithDetails & { autoPrepsCreated?: { recipeId: number; title: string; servings: number }[] }> {
    const plannedDate = new Date(data.plannedDate + 'T12:00:00');
    
    if (!data.recipeId) {
      throw new Error('Debe especificar recipeId');
    }
    ...
    const autoPrepsCreated: { recipeId: number; title: string; servings: number }[] = [];
    ...
    if (planType === 'meal') {
      ...
      for (const meal of plannedMeals) {
        ...
        for (const comp of meal.recipe.components || []) {
          ...
          if (mealSelections.length > 0) {
            ...
            if (selectedSel) {
              selectedOption = comp.options.find((o: any) => o.id === selectedSel.optionId);
            } else if (comp.isOptional) {
              continue;
            } else {
              selectedOption = comp.options.find((o: any) => o.isDefault) || comp.options[0];
            }
          } else {
            selectedOption = comp.options.find((o: any) => o.isDefault) || comp.options[0];
          ...
          if (selectedOption?.recipe) {
            ...
            const usedServings = (selectedOption.recipeServings || 1) * mealRatio;
      ...
      // Only check SUB-RECIPES in components (not the main recipe)
      // The main recipe is cooked from ingredients, not prepared separately
      for (const comp of recipe.components || []) {
        ...
        if (selections.length > 0) {
          ...
          if (selectedOptId) {
            selectedOption = comp.options.find((o: any) => o.id === selectedOptId);
          } else if (comp.isOptional) {
            continue;
          } else {
            selectedOption = comp.options.find((o: any) => o.isDefault) || comp.options[0];
        ...
        if (selectedOption.recipe) {
          ...
          if (available < neededServings) {
            ...
            availableServings.set(subRecipe.id, (available || 0) + (subRecipeServings * batchesNeeded) - neededServings);
          }
        }
      }
    }
  ...
  private async addIngredientsToShoppingList(weekPlanId: number, userId: number) {
  ...
  async updatePlanDate(id: number, newDate: Date, userId: number): Promise<WeekPlanWithDetails | null> {
    const plan = await prisma.weekPlan.findFirst({
      where: { id, userId }
    });
  ...
  async generateShoppingList(userId: number, startDate: Date, endDate: Date): Promise<ShoppingItem[]> {
    ...
    const ingredientNeeds = new Map<number, { rawEquivalent: number; name: string; ingredientData: any }>();
    ...
    for (const plan of plans) {
      ...
      for (const ri of plan.recipe.ingredients || []) {
        ...
        const quantityInGrams = this.convertToBaseUnit(ri.quantity * ratio, usedUnit, ri.ingredient);
        ...
        console.log(`[SHOPPING] ${ri.ingredient.name}: qty=${ri.quantity}, ratio=${ratio}, inGrams=${quantityInGrams}, variant=${(ri as any).variant?.name}, wf=${variantWeightFactor}, rawEq=${rawEquivalent}`);
      ...
      for (const comp of plan.recipe.components || []) {
        let selectedOption;
        if (selections.length > 0) {
          const sel = selections.find((s: any) => comp.options.some((o: any) => o.id === s.optionId));
          if (sel) {
            selectedOption = comp.options.find((o: any) => o.id === sel.optionId);
          } else if (comp.isOptional) {
            continue;
          } else {
            selectedOption = comp.options.find((o: any) => o.isDefault) || comp.options[0];
        ...
        if (selectedOption?.ingredient) {
          const usedUnit = selectedOption.unit || selectedOption.ingredient.unit;
          ...
          const variantWeightFactor = (selectedOption as any).variant?.weightFactor || 1;
          const rawEquivalent = quantityInGrams / variantWeightFactor;
    ...
    for (const [ingredientId, { rawEquivalent, name, ingredientData }] of ingredientNeeds) {
      ...
      if (ingredientData.preferredUnit && toBuyRaw > 0) {
        ...
        const conversion = conversions.find((c: any) => 
          c.unitName.toLowerCase() === ingredientData.preferredUnit.toLowerCase()
        );
        
        if (conversion && conversion.gramsPerUnit > 0) {
          preferredUnit = ingredientData.preferredUnit;
          preferredQuantity = Math.round((toBuyRaw / conversion.gramsPerUnit) * 10) / 10;
        }
      }
    ...
    return result.filter(item => item.quantityToBuy > 0).sort((a, b) => a.name.localeCompare(b.name));
  }

  // Convertir cantidad a unidad base (g o ml)
  private convertToBaseUnit(quantity: number, usedUnit: string, ingredient: any): number {
  ...
  async markShoppingItemPurchased(ingredientId: number, userId: number): Promise<void> {
  ...
  private addRecipeIngredients(
    ...
    for (const comp of recipe.components || []) {
      ...
      if (selections.length > 0) {
        ...
        if (selectedOption) {
          option = comp.options.find((o: any) => o.id === selectedOption.optionId);
        } else if (comp.isOptional) {
      ...
      if (option.recipe) {
        ...
        if (adjustedRatio > 0) {
          this.addRecipeIngredients(option.recipe, adjustedRatio, map, [], homeRecipeServings);
        }
  ...
  async markAsCooked(
  ...
  ): Promise<{ success: boolean; ingredientsDeducted: number; leftoversSaved: boolean }> {
    const plan = await prisma.weekPlan.findFirst({
    ...
    const ingredientsNeeded = new Map<number, { quantity: number; unit: string }>();
    
    if (plan.recipe) {
      const ratio = plan.servings / plan.recipe.servings;
      this.collectIngredients(plan.recipe, ratio, ingredientsNeeded, plan.selections || []);
    }
  ...
  private mapWeekPlanWithDetails(plan: any): WeekPlanWithDetails {
    return {
      ...
      selections: plan.selections ? plan.selections.map((s: any) => ({
        optionId: s.optionId,
        option: {
          id: s.option.id,
          name: s.option.name,
